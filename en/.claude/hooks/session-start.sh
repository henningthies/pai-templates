...
check_setup() {
    if [[ -f "$CLAUDE_MD" ]]; then
        if grep -qE '\[.*\.\.\.\]|\[Your.*\]|\[your.*\]|\[e\.g\.' "$CLAUDE_MD"; then
            return 1
        fi
    else
        return 1
...
get_current_tip() {
    if [[ -f "$PROGRESS_FILE" ]]; then
        local tip_num=$(grep -oP 'Current Tip:\s*\K\d+' "$PROGRESS_FILE" 2>/dev/null || echo "0")
        echo "$tip_num"
    else
        echo "0"
    fi
...
if ! check_setup; then
    echo "🔧 SETUP REQUIRED"
    echo "CLAUDE.md still contains placeholders."
    echo ""
    echo "[ACTION FOR CLAUDE: Offer to run /pai:setup]"
    echo ""
else
    # Check standup
    if ! check_standup; then
        echo "📋 TODAY'S STANDUP: Not done yet"
        echo ""
        echo "[ACTION FOR CLAUDE: Ask if we should do a quick standup now]"
        echo ""
    else
        echo "✅ Today's standup: Done"
        echo ""
    fi
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "[INSTRUCTION: Greet the user based on the info above and handle the marked ACTIONS]"
